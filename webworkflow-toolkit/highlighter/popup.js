...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function handleFileImport(event) {
    console.log('[IMPORT] handleFileImport called');
    ...
    reader.onload = function(e) {
      ...
      try {
        ...
        console.log('[IMPORT] Settings validation passed');
        ...
        console.log('[IMPORT] Settings extracted:');
        console.log('[IMPORT]   - keywords:', keywords.length, 'items', keywords);
        console.log('[IMPORT]   - urlPatterns:', urlPatterns);
        console.log('[IMPORT]   - highlightColor:', highlightColor);
        console.log('[IMPORT]   - elementSelector:', elementSelector);
        console.log('[IMPORT]   - useWordBoundaries:', useWordBoundaries);

        // Update UI
        colorPicker.value = highlightColor;
        selectorInput.value = elementSelector;
        wordBoundariesCheckbox.checked = useWordBoundaries;
        console.log('[IMPORT] UI controls updated');

        renderKeywordList();
        console.log('[IMPORT] Keyword list rendered');

        renderUrlList();
        console.log('[IMPORT] URL list rendered');

        // Automatically save imported settings to storage
        console.log('[IMPORT] Attempting to save to browser.storage.local...');
        const dataToSave = {
          keywords: keywords,
          urlPatterns: urlPatterns,
          highlightColor: highlightColor,
        ...
        console.log('[IMPORT] Data to save:', dataToSave);

        browser.storage.local.set(dataToSave).then(function() {
          console.log('[IMPORT] ✓ Storage save successful!');

          // Verify save by reading back
          browser.storage.local.get(['keywords', 'highlightColor', 'urlPatterns', 'elementSelector', 'useWordBoundaries']).then(function(result) {
            console.log('[IMPORT] ✓ Verification read from storage:', result);
            showStatus('Settings imported and saved successfully! Reload pages for URL changes to take effect.', 'success');
            setTimeout(() => {
              statusDiv.textContent = '';
            }, 5000);
          });
        }).catch(function(error) {
          console.error('[IMPORT] ✗ Error auto-saving imported settings:', error);
          showStatus('Settings imported but failed to save: ' + error.message, 'error');
          setTimeout(() => {
            statusDiv.textContent = '';
          }, 5000);
        });
